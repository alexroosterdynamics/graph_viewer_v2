...
export default function Page() {
  ...
  // Toggle option:
  // - if placeholder -> convert to real (magenta), start naming (lock others)
  // - if already created (magenta) -> revert to placeholder (gray), stop naming
      ...
      const nodes = g.nodes.map((n) => {
        if (n.id === pid && n.__placeholder) {
          ...
          return {
            ...
            fy: ny,
            static: false,
            name: "", // to be typed; label hidden until set
          };
        }
        return n;
      });
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-gray-900 text-white">
      ...
      <SelectionOverlay
        ...
        lockedKey={lockedKey} // show active (magenta) + disable others while naming
      />
        ...
        (() => {
          ...
          return (
            <form
              ...
              style={{ left: p.x, top: p.y - 10 }} // a few px above node center
            >
              <input
                ref={inputRef}
                name="nm"
      ...
      <ForceGraph2D
        ...
        nodeCanvasObject={(node, ctx, globalScale) => {
          ...
          // Big inner glow (center very bright, edges notably darker)
          ...
          ctx.beginPath();
          // wide arc across the top hemisphere
          ctx.arc(
            x,
            y,
            r * 0.78,
          ...
          if (LABELS.background) {
            ...
            const h = px;
            // single subtle glass plate (no borders)
